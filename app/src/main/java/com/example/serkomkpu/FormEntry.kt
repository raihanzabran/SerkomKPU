...
import com.example.serkomkpu.camera.CameraActivity
import com.example.serkomkpu.camera.rotateBitmap
import com.example.serkomkpu.camera.uriToFile
import com.example.serkomkpu.databinding.ActivityFormEntryBinding
import com.example.serkomkpu.db.PemilihDB
import com.example.serkomkpu.helper.EntryViewModel
...
class FormEntry : AppCompatActivity() {
    ...
    private var datapemilih: PemilihDB? = null
    private lateinit var formEntryViewModel: EntryViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isEdit) {
            ...
            if (datapemilih != null) {
                datapemilih?.let { datapemilih ->
                    ...
                    if (datapemilih?.foto != null) {
                        val bitmap = BitmapFactory.decodeByteArray(datapemilih?.foto, 0, datapemilih?.foto?.size ?: 0)
                        binding?.previewImageView?.setImageBitmap(bitmap)
                    } else {
                        Glide.with(applicationContext)
                            .load(R.drawable.take_photo) // Ganti dengan resource ID placeholder yang sesuai
        ...
        binding?.btnSubmit?.setOnClickListener {
            ...
            formEntryViewModel.getPemilihByNIK(nik).observe(this) { existingDataPemilih ->
                                ...
                                if (alamat.isEmpty()) {
                                    ...
                                    datapemilih.let { datapemilih ->
                                        ...
                                        if (getFile != null) {
                                            ...
                                            datapemilih?.foto = imageByteArray
                                        }
                                    }

                                    if (isEdit) {
                                        formEntryViewModel.update(datapemilih as PemilihDB)
                                        showToast("Satu item berhasil diubah")
                                    } else {
                                        formEntryViewModel.insert(datapemilih as PemilihDB)
    ...
    private fun showAlertDialog(type: Int) {
        ...
        with(alertDialogBuilder) {
            ...
            setPositiveButton("Ya") { _, _ ->
                if (!isDialogClose) {
                    formEntryViewModel.delete(datapemilih as PemilihDB)
    ...
    private val launcherIntentCameraX = registerForActivityResult(
        ...
        if (it.resultCode == CAMERA_X_RESULT) {
            ...
            val path = MediaStore.Images.Media.insertImage(this@FormEntry.contentResolver, result, "Title", null)
            val uri = Uri.parse(path.toString())
            getFile = uriToFile(uri, this@FormEntry)
    ...
    private val launcherIntentGallery = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            ...
            val myFile = uriToFile(selectedImg, this@FormEntry)
    ...
    private fun obtainViewModel(activity: AppCompatActivity): EntryViewModel {
        val factory = ViewModelFactory.getInstance(activity.application)
        return ViewModelProvider(activity, factory).get(EntryViewModel::class.java)
    }
}